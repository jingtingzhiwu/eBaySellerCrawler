...
public class ProxyPersistUtils {
	...
	public Map<String, HttpProxy> read() {
		...
		if (content != null && content.length() > 0) {
			...
			for (String line : lines) {
				...
				HttpProxy httpProxy = new HttpProxy(proxy, Integer.parseInt(tmps[2]), Integer.parseInt(tmps[3]), Integer.parseInt(tmps[4]));
	...
	public void save(Map<String, HttpProxy> totalQueue) {
		...
		for (Map.Entry<String, HttpProxy> entry : totalQueue.entrySet()) {
			...
			stringBuilder.append(address.getAddress().getHostAddress()).append(":").append(address.getPort()).append(":").append(entry.getValue().getBorrowNum()).append(":")
					.append(entry.getValue().getFailedNum()).append(":").append(entry.getValue().getReuseTimeInterval()).append("\n");
		}
		try {
